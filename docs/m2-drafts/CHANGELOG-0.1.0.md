# 变更日志（T12 草案 — 供主对话终审后落盘为根 `CHANGELOG.md`）

> 素材：`docs/m1-report.md`、`docs/m1b-report.md`、`docs/m2-tasks.md` §9 与 M2 各任务提交信息。
> 日期与最终措辞由主对话终审确定；本文件不直接落盘根 `CHANGELOG.md`。
...
- sanitizer 清扫：ASan/UBSan 全语料矩阵零发现；TSan 并发路径 0 报告（抑制文件附 happens-before 论证
  与阳性对照）
- GitHub Actions 工作流：`build-test`（构建 + ctest + 金样/回归）、`ui-smoke`（offscreen 无头冒烟）、
  `appimage`（打包 + 烟测 + 产物入 artifact）
