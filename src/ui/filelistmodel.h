...
//
// PP-THAWED(0.3.0-M4-D20) —— 解冻裁定表 §3.6 行 `ui/filelistmodel.h`（依据 docs/v0.3.0-design.md
// §3.6）
//   裁定原文（本行为逐字抄录；行首 "// " 为注释包装）：
// clang-format off
// | `ui/filelistmodel.h` | 追加 roles：`CheckState`（Qt 标准）、`ClassId`、`GroupKey`；模型 QListView→QTreeView（分组节） |
// clang-format on
//   落地任务 = W2-T11（文件列表/分类：勾选 + 正交搜索 + 自动分组 QTreeView；出口含 UI
//   冒烟扩展段绿、 `classes.json` 往返）→ 落地后改标 PP-FROZEN(0.3.0)。 本文件其余既有
//   roles/声明不在本表行内 → 维持 PP-FROZEN(M1b) 只读（仅上面 3 个 role 与视图形态属解冻面）。
#pragma once
...
// 追加 roles：`CheckState`（Qt 标准）、`ClassId`、`GroupKey`；模型 QListView→QTreeView（分组节）
    // clang-format on
    //   建议形态（§3.6 未逐字给出 role 序号 → 追加在既有 roles 末尾，Qt 标准枚举直接用
    //   `Qt::CheckStateRole`）：
    // clang-format off
    ...
    //   注（落地待定，T11 处置）：模型现形基类为 `QAbstractListModel`（见本文件 `class
    //   FileListModel` 声明行；行号随注释变动，勿以行号定位）， "QListView→QTreeView（分组节）"
    //   需层次结构 → 基类/节行的落地形态由 T11 定（M4-T1 已上报，W5 收口入 m4-report）。
    enum Roles {
