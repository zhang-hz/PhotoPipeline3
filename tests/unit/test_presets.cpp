...
std::string diff_edits(const char* what, const std::vector<pp::TagEdit>& a,
    ...
    if (a.size() != b.size())
        return std::string(what) + " size " + std::to_string(a.size()) + " != " + std::to_string(b.size());
...
std::string diff_preset(const pp::PresetData& a, const pp::PresetData& b) {
    ...
    if (a.format_id != b.format_id) return "format '" + a.format_id + "' != '" + b.format_id + "'";
    if (a.backend_id != b.backend_id) return "backend '" + a.backend_id + "' != '" + b.backend_id + "'";
    if (a.tech_id != b.tech_id) return "tech '" + a.tech_id + "' != '" + b.tech_id + "'";
    if (a.lossless != b.lossless) return "lossless";
    if (a.out_bitdepth != b.out_bitdepth) return "bitdepth";
    if (a.color_target != b.color_target) return "color_target";
    if (a.conflict != b.conflict) return "conflict";
    if (!(a.params == b.params)) {
        std::string d = "params differ:";
        for (const auto& [k, v] : a.params)
            if (!b.params.count(k)) d += " <" + k;
        for (const auto& [k, v] : b.params)
            if (!a.params.count(k)) d += " >" + k;
        return d;
    }
    if (a.rules.time_shift.has_value() != b.rules.time_shift.has_value()) return "rules.time_shift presence";
    if (a.rules.time_shift && !same_timeshift(*a.rules.time_shift, *b.rules.time_shift))
        return "rules.time_shift fields";
    if (a.rules.gps.has_value() != b.rules.gps.has_value()) return "rules.gps presence";
    if (a.rules.gps && !same_gps(*a.rules.gps, *b.rules.gps)) return "rules.gps fields";
    if (a.rules.gps_clear != b.rules.gps_clear) return "rules.gps_clear";
    if (a.rules.strip_privacy != b.rules.strip_privacy) return "rules.strip_privacy";
    if (a.rules.sync_mtime != b.rules.sync_mtime) return "rules.sync_mtime";
    if (const std::string d = diff_edits("rules.exif_edits", a.rules.exif_edits, b.rules.exif_edits);
        !d.empty())
        return d;
...
int main() {
    ...
    // 1) 预设往返：unicode name + rules 各字段（delta）
    {
        ...
        for (const char* k : {"version", "name", "format", "backend", "tech", "lossless", "bitdepth",
                              "color_target", "conflict", "params", "rules"})
        ...
        check(o.value("color_target").toString() == "p3" && o.value("conflict").toString() == "rename",
        ...
        check(params.value("distance").toDouble(-1) == 0.0 && params.value("effort").toInteger(-1) == 9 &&
        ...
        for (const char* k : {"time_shift", "gps", "gps_clear", "exif_edits", "xmp_edits",
            ...
            check(rules.contains(QLatin1String(k)), c, std::string("missing rules key '") + k + "'");
        const QJsonObject ts = rules.value("time_shift").toObject();
        check(ts.value("mode").toString() == "delta" && ts.value("years").toInteger(-1) == 1 &&
                  ts.value("seconds").toInteger(-1) == 6 && ts.value("from_offset_min").toInteger(-1) == 0 &&
        ...
        check(gps.value("lat").toDouble() == 31.2304 && gps.value("lon").toDouble() == 121.4737 &&
                  gps.value("altitude").toDouble(-1) == 12.5 && gps.value("direction").toDouble(-1) == 359.99 &&
        ...
        check(rules.value("strip_privacy").toBool(false) && rules.value("sync_mtime").toBool(false), c,
              "privacy/mtime flags");
    ...
    // 2) 预设往返：时区语义 TimeShift + gps 缺省 + gps_clear
    {
        ...
        check(tsj.value("mode").toString() == "timezone" && tsj.value("from_offset_min").toInteger(-1) == 480 &&
    ...
    // 3) 保留键不落盘（顶层 lossless 表达）
    {
        ...
        const std::string err = ui::save_preset(file, p);
        check(err.empty(), c, "save: " + err);
        check(read_file(file).find("__lossless") == std::string::npos, c, "reserved key leaked to JSON");
        PresetData q;
    ...
    // 5) validate_preset：格式/后端/技术/位深/参数/版本
    {
        ...
        bad = p;
        bad.backend_id = "nope";
        check(validate_preset(bad).find("backend") != std::string::npos, c, "unknown backend must fail");
        bad = p;
        bad.tech_id = "nope";
        check(validate_preset(bad).find("tech") != std::string::npos, c, "unknown tech must fail");
        bad = p;
        bad.out_bitdepth = 12;
        check(validate_preset(bad).find("bitdepth") != std::string::npos, c, "bad bitdepth must fail");
        bad = p;
        bad.params["effort"] = int64_t(42);
        check(validate_preset(bad).find("effort") != std::string::npos, c, "out-of-range param must fail");
        bad = p;
    ...
    // 6) normalize_preset：补齐后端/技术/参数 + 应用锁定
    {
        ...
        check(validate_preset(p).empty(), c, "normalized preset must validate: " + validate_preset(p));
        PresetData bad;
        bad.format_id = "nope";
    ...
    // 7) I/O 错误路径
    {
        ...
        check(ui::save_preset(nested, p).empty() && fs::exists(nested), c, "save must create parent dirs");
        ...
        check(ui::load_preset(badconf, out).find("conflict") != std::string::npos, c, "bad conflict");
        // 缺省字段 → 默认值（向前兼容）
