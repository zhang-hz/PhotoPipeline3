...
void collect_from(const std::filesystem::path& input, std::vector<std::filesystem::path>& files,
    ...
    if (std::filesystem::is_directory(input, ec)) {
        ...
        if (ec) return;   // 目录打不开（权限/竞态）→ 忽略该输入项
        const std::filesystem::recursive_directory_iterator end;
        for (; it != end; it.increment(ec)) {
            if (ec) break;   // 目录项枚举失败 → 停止该子树
            const std::filesystem::path& p = it->path();
            const std::string name = p.filename().string();
            if (!name.empty() && name.front() == '.') {   // 点文件/点目录
                std::error_code dir_ec;
                if (it->is_directory(dir_ec)) it.disable_recursion_pending();
                continue;
            }
...
QVariant FileListModel::data(const QModelIndex& idx, int role) const {
    if (!idx.isValid() || idx.row() < 0 || std::size_t(idx.row()) >= rows_.size()) return {};
...
void FileListModel::add_paths(const QStringList& paths) {
    ...
    for (const std::filesystem::path& f : files) {
        ...
        if (known_paths_.contains(key) || !batch_keys.insert(key).second) continue;
        fresh.push_back(f);
    }
    ...
    if (!fresh.empty() || unsupported != 0) emit content_changed();
}

void FileListModel::remove_rows(const QList<int>& rows) {
    if (rows.isEmpty() || rows_.empty()) return;

    std::vector<int> idx;
    idx.reserve(std::size_t(rows.size()));
    for (int r : rows) {
        if (r >= 0 && std::size_t(r) < rows_.size()) idx.push_back(r);
    }
    if (idx.empty()) return;
...
void FileListModel::clear() {
    if (thumbs_) thumbs_->clear_pending();   // 在途结果 → stale（apply_thumb 亦按越界丢弃）
...
std::vector<pp::FileEntry> FileListModel::entries() const {
    ...
    for (const FileRow& r : rows_) out.push_back(r.entry);   // 值语义深拷贝（含 exception）
    return out;
}

void FileListModel::set_state(std::size_t i, pp::FileState st) {
    if (i >= rows_.size() || rows_[i].state == st) return;
    rows_[i].state = st;
...
void FileListModel::set_exception(std::size_t i, std::optional<pp::MetadataOverride> ex) {
    ...
    if (had == r.has_exception() && !had) return;   // 无变化（都为空）
    const QModelIndex idx = index(int(i));
...
void FileListModel::apply_thumb(int row, const QString& path, const pp::ImageInfo& info,
    ...
    if (row < 0 || std::size_t(row) >= rows_.size()) return;   // stale：行越界（删行/清空）
    FileRow& r = rows_[std::size_t(row)];
    if (norm_string(path) != fs_string(r.entry.src)) return;   // stale：path 不匹配（行漂移）
    ...
    emit dataChanged(idx, idx, {ThumbRole, DimRole, FormatRole, StateRole, StateTextRole, ErrorRole});
    if (!probe_ok && !was_failed) emit content_changed();
}

void FileListModel::set_thumbnailer(Thumbnailer* t) {
    if (thumbs_ == t) return;
    if (thumbs_) disconnect(thumbs_, nullptr, this, nullptr);
    thumbs_ = t;
    if (!thumbs_) return;
...
void FileListModel::enqueue_pending_thumbs() {
    ...
    for (std::size_t i = 0; i < rows_.size(); ++i) {
        ...
        if (r.probe_ok || !r.error.isEmpty()) continue;   // 已有结果 / 已判失败
...
void FileDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
    ...
    if (const QAbstractItemView* view = qobject_cast<const QAbstractItemView*>(widget)) {
        ...
        if (area.right() > visible_right) area.setRight(visible_right);
    }
    ...
    const QColor secondary =
        selected ? opt.palette.color(QPalette::HighlightedText) : QColor(QLatin1String(kSubLineColor));
    ...
    if (!format.isEmpty()) sub = sub.isEmpty() ? format : sub + QStringLiteral(" · ") + format;
    ...
    if (text_width > 0) {
        ...
        if (!sub.isEmpty()) {
            ...
            painter->drawText(QRect(text_left, area.top() + line_h, text_width, area.height() - line_h),
                              Qt::AlignLeft | Qt::AlignVCenter,
