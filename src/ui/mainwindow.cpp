...
//   * FileListModel::set_thumbnailer 内部已 connect(ready→apply_thumb) 并自动 enqueue → 不重复连接；
...
QString conflict_label(pp::ConflictPolicy p) {
    switch (p) {
        case pp::ConflictPolicy::Rename: return T("自动加序号");
        case pp::ConflictPolicy::Skip: return T("跳过");
        case pp::ConflictPolicy::Overwrite: return T("覆盖");
    }
    return T("自动加序号");
}
...
bool MainWindow::Impl::handle_event(QObject* /*watched*/, QEvent* event) {
    switch (event->type()) {
        ...
        case QEvent::Drop: {
            ...
            if (e->mimeData() == nullptr) return false;
            QStringList paths;
...
void MainWindow::wire() {
    ...
    connect(d.view, &QListView::doubleClicked, this, [this](const QModelIndex& idx) {
        if (idx.isValid()) open_exif_editor_row(impl_->proxy->mapToSource(idx).row());
    });
    ...
    connect(d.thumbs, &Thumbnailer::ready, this,
            [this](int, const QString&, const pp::ImageInfo& info, bool ok, const QString&,
                   const QImage&) {
                if (!ok || !info.has_alpha || impl_->running || impl_->cached_alpha) return;
    ...
    connect(d.page_meta, &PageMeta::open_editor_requested, this, &MainWindow::open_exif_editor_path);
    ...
    connect(d.page_output, &PageOutput::manage_presets_requested, this, &MainWindow::manage_presets);
...
void MainWindow::set_current_page(int one_based) {
    ...
    if (action != nullptr && !action->isChecked()) action->setChecked(true);
}

void MainWindow::set_offline_maps(bool off) {
    const QList<pp::map::MapWidget*> maps = findChildren<pp::map::MapWidget*>();
    for (pp::map::MapWidget* map : maps) map->set_offline(off);
}
...
void MainWindow::on_start() {
    ...
    if (cfg.metadata_only) {
        ...
        for (std::size_t i = 0; i < d.model->size(); ++i) {
            ...
            if (!is_metadata_only_format(suffix)) bad << name;
        }
        if (!bad.isEmpty()) {
    ...
    for (std::size_t i = 0; i < total; ++i) d.model->set_state(i, pp::FileState::Queued);
    d.run_total = total;
    d.run_terminal = 0;
    d.run_done = 0;
    d.run_failed = 0;
...
void MainWindow::on_cancel() {
    if (impl_->sched) impl_->sched->cancel();   // 幂等；进行中的文件跑完
}
...
void MainWindow::lock_for_run(bool lock) {
    ...
    if (lock) set_current_page(3);   // stack 锁到运行页
    refresh_status();
}
...
void MainWindow::refresh_status() {
    ...
    if (exceptions > 0) text += tr(" · %1 个例外").arg(exceptions);
...
void MainWindow::open_settings() {
    ...
    if (!parse_log_level_text(d.settings.log_level, level)) level = pp::LogLevel::Info;
...
void MainWindow::manage_presets() {
    ...
    PresetsDialog dlg(items,
                      d.last_preset_path.isEmpty()
                          ? QString()
                          : QFileInfo(d.last_preset_path).completeBaseName(),
                      this);
    if (dlg.exec() != QDialog::Accepted) return;
    ...
    switch (dlg.action()) {
        ...
        case PresetsDialog::Action::Delete: {
            ...
            if (d.last_preset_path == path) d.last_preset_path.clear();
            return;
        }
...
void MainWindow::open_exif_editor_row(int row) {
    Impl& d = *impl_;
    if (d.running) return;
    if (row < 0 || static_cast<std::size_t>(row) >= d.model->size()) return;
...
void MainWindow::Impl::restore_session() {
    ...
    page_meta->set_map_provider(QString::fromStdString(settings.map_provider),
                                QString::fromStdString(settings.amap_key),
                                settings.tile_cache_mb);
    // 预设优先于 last_format（§2.14）：仅当文件仍存在
    if (settings.last_preset.empty()) return;
    std::error_code ec;
    if (!std::filesystem::exists(settings.last_preset, ec) || ec) return;
    pp::PresetData preset;
    if (!pp::ui::load_preset(settings.last_preset, preset).empty()) return;
    pp::normalize_preset(preset);
    ...
    if (!running) {
        ...
        if (paths != cached_paths) {
            ...
            for (std::size_t i = 0; i < model->size(); ++i) {
                ...
                if (row.probe_ok && row.info.has_alpha) any_alpha = true;
            }
...
void MainWindow::Impl::sync_selection() {
    ...
    if (view->selectionModel() != nullptr) {
        for (const QModelIndex& idx : view->selectionModel()->selectedIndexes()) {
            ...
            if (row < 0 || static_cast<std::size_t>(row) >= model->size()) continue;
...
void MainWindow::Impl::remove_selected() {
    if (view->selectionModel() == nullptr) return;
    QList<int> rows;
    for (const QModelIndex& idx : view->selectionModel()->selectedIndexes()) {
        const int row = proxy->mapToSource(idx).row();
        if (row >= 0 && !rows.contains(row)) rows << row;
    }
...
QString MainWindow::Impl::repo_root() {
    static const QString cached = [] {
        ...
        for (int i = 0; i < 8; ++i) {
            if (dir.exists(QStringLiteral("CMakeLists.txt")) || dir.exists(QStringLiteral(".git"))) {
...
void MainWindow::Impl::smoke_amap_boundary(pp::map::MapWidget* map) {
    ...
    const QMetaObject::Connection conn =
        QObject::connect(map, &pp::map::MapWidget::point_selected, w,
                         [&lat, &lon, &hits](double la, double lo) {
                             lat = la;
                             lon = lo;
                             ++hits;
                         });
    ...
    if (hits != 1) {
        smoke_fail(MainWindow::tr("amap 边界：视口中心点击未产生 point_selected（hits=%1）")
                       .arg(hits));
    } else {
...
void MainWindow::Impl::smoke_run(const QString& shots_dir) {
    ...
    if (page_output->current_format() != QLatin1String("jxl")) {
        smoke_fail(MainWindow::tr("冒烟前置：格式不是 jxl（%1）").arg(page_output->current_format()));
    }
    ...
    QProgressBar* run_progress = w->findChild<QProgressBar*>(QStringLiteral("runProgress"));
    {
        ...
        while (spin.elapsed() < 60) {
            ...
            if (page_run->is_running() && maximum > 0 && value > 0 && value < maximum) break;
            QThread::msleep(1);
        }
    }
    ...
    if (QLabel* summary_text = w->findChild<QLabel*>(QStringLiteral("runSummaryText"))) {
        const QString text = QString(summary_text->text()).replace(QLatin1Char('\n'),
                                                                   QLatin1String(" / "));
