...
- 构建/验证（本任务专属）：`cmake --preset release-dev -DVCPKG_MANIFEST_INSTALL=OFF && cmake --build --preset release-dev -j` → `ctest --test-dir build/release-dev --output-on-failure` 全绿（**24 条** = 旧 24 − offscreen + ui_smoke）→ `--ui-smoke --shots .cache/ui-review` 产出 **8 张** PNG（01/02/02b/03/03b/04/05/06，与上方走查清单一致——**2026-09-20 勘误**：原文"7 张"系笔误，主对话曾误抄进 U10 指令，任务书 8 张清单为准）并报告路径。
...
- 03-run 时点：16 小文件 ~150ms 跑完，开始后自旋 ≤60ms 再抓（实测 12–16/16 运行中）；偶已跑完则与 03b 同图，仅 stdout 记录 running 计数，不算失败。
- 引擎侧噪音（未授权修，M2 备忘）：`Scheduler::wait()` 无 elapsed 守卫，每批打 2 行 "budget report"（UI wait + 析构 wait）。
- G5 实证：320 文件 workers=1 取消测试 → 运行中左面板/输出页禁用、stack 锁运行页；取消后 "已取消：完成 33 · 取消 287"、解锁、摘要可见。
...
**修正项（各任务域）**：
- **U3 paramform**：[高] 参数行的 `lossless` 键**一律不渲染为行**（专属"无损"复选框拥有该参数）。**落地事实（566f888）**：webp 静态技术本就无 lossless 键（行数不变，21）；实际 -1 的是 heif 7→6、avif 15→14；且复选框**独占该键值**——refresh() 内 `values["lossless"]=sel.lossless`（键存在时），否则 enc_heif.cpp:373 读不到该参数使"无损"变空操作。[低] "搜索参数…" 输入框宽度与相邻字段一致（勿拉满整行）；[低] "高级参数"组标题与搜索框间距收紧。
...
- **U7 page_meta**：[中] 卡片"启用"未勾选时卡内其余控件 setEnabled(false)（时间偏移/GPS 两卡；"清除 GPS"复选框与 MapWidget 本体不门控。**标签修改卡无启用开关，不门控**——其激活模型 = 行本身，2026-09-20 裁定维持）。
