...
| T30 | **计数口径稳定化 + 推送 + 终轮 CI 验证**（**仅文档**：`docs/m2-report.md` / `docs/m2-tasks.md`）：全部"累计提交 / M2 批次提交"表述改以**发行提交 `4fb64c3`（tag `v0.1.0`）**为稳定锚点 —— **累计提交 155**（`git rev-list --count 4fb64c3`）、**M2 批次提交 55**（`git rev-list --count ab5a88a^..4fb64c3`）；**不再按会随文档轮漂移的 HEAD 记数**（`4fb64c3` 之后仅文档提交，不影响发行产物与 tag）；**§9 批次表按同一锚点统一重算**（M0 16 / M1a 44 / M1b 40 / M2 55 = 155）；推送 2 次（`4fb64c3..6f61abd`、`6f61abd..87acab0`）；tag 后文档修订链 `672f695`→`6f61abd`→`87acab0`，**本文档不声明"最终提交 hash"，当前状态一律以 `git log origin/main` 为准**；**终轮 CI = run `35729929908`（head `6f61abd`）四 job 全绿**（`linux` 1m48s / `ui-smoke` 1m47s / `appimage` 1m57s / `cache-gc` 8s），冻结行原文 `UI-SMOKE OK shots=8 pages=3`；**末轮文档轮 run `35730278620`（head `87acab0`）conclusion=success**（`linux` 2m05s / `ui-smoke` 1m30s / `appimage` 1m59s / `cache-gc` 9s）；**以上均为 tag 后的文档轮，产物非发布附件**；详见 `docs/m2-report.md` §5⑦ | — |
