...
- 单测 test_gcj02（仅依赖 coord.h，链 pp_core）：北京(39.9042,116.4074)/上海(31.2304,121.4737)/广州(23.1291,113.2644)：wgs→gcj 偏移量级 300–600m（度距×111320 估算）；gcj(wgs(p)) 往返误差 <1e-6 度；中国外坐标恒等（如 (40.7128,-74.0060) 纽约）。
- MapWidget 本体不写单测（offscreen 网络交互留给 U10 冒烟）；`set_offline(true)` 时构造/绘制不得崩溃（构造后 `grab()` 非空——写进 test_gcj02 里顺带断言一次：qputenv offscreen + QApplication + MapWidget offline + resize(320,240) + grab）。
