...
#endif
    if (f == nullptr)
        return "cannot open '" + display_path(file) + "' for reading";
    std::string bytes;
    char buf[64 * 1024];
    std::string err;
    for (;;) {
        ...
        if (n == sizeof(buf))
            continue;
        if (std::ferror(f) != 0)
            err = "cannot read '" + display_path(file) + "': " + errno_text();
        break;
    }
    std::fclose(f);
    if (!err.empty())
...
std::string directory_target_error(const std::filesystem::path &file, const char *action) {
    ...
    if (!std::filesystem::is_directory(file, dir_ec))
        return {};
    ...
    if (def) {
        ...
        case ParamType::Enum:
            if (!def->choices.empty()) {
                ...
                if (std::holds_alternative<bool>(proto))
                    return ParamValue{v.toBool(false)};
            }
            break;
        }
...
QJsonObject edit_to_json(const TagEdit &e) {
    ...
    o.insert(QStringLiteral("value"),
             e.value ? QJsonValue(qs(*e.value)) : QJsonValue(QJsonValue::Null));
    if (e.remove)
        o.insert(QStringLiteral("remove"), true); // 仅在 true 时写出（示例形态）
    return o;
}
...
QJsonObject rules_to_json(const BatchRules &r) {
    ...
    if (r.time_shift) {
        ...
        QJsonObject ts;
        ts.insert(QStringLiteral("mode"), t.mode == TimeShift::Mode::TimezoneSemantic
    ...
    QJsonArray exif;
    for (const TagEdit &e : r.exif_edits)
        exif.append(edit_to_json(e));
    o.insert(QStringLiteral("exif_edits"), exif);
    QJsonArray xmp;
    for (const TagEdit &e : r.xmp_edits)
        xmp.append(edit_to_json(e));
...
bool rules_from_json(const QJsonObject &o, BatchRules &r, std::string &err) {
    ...
    if (!ts_v.isUndefined() && !ts_v.isNull()) {
        if (!ts_v.isObject()) {
            err = "rules.time_shift is not an object";
            return false;
        }
        const QJsonObject ts = ts_v.toObject();
        TimeShift t;
        const std::string mode =
            utf8(ts.value(QStringLiteral("mode")).toString(QStringLiteral("delta")));
        if (mode == "timezone" || mode == "timezone_semantic")
            t.mode = TimeShift::Mode::TimezoneSemantic;
        else if (mode == "delta")
            t.mode = TimeShift::Mode::Delta;
        else {
            err = "rules.time_shift.mode '" + mode + "' is not valid";
            return false;
        }
        ...
        t.from_offset_min =
            static_cast<int>(ts.value(QStringLiteral("from_offset_min")).toInteger(0));
    ...
    if (!gps_v.isUndefined() && !gps_v.isNull()) {
        if (!gps_v.isObject()) {
            err = "rules.gps is not an object";
            return false;
        }
...
std::string save_preset(const std::filesystem::path &file, const PresetData &p) {
    ...
    if (const std::string derr = directory_target_error(file, "writing"); !derr.empty())
        return derr;
    std::error_code ec;
    ...
    if (!parent.empty()) {
        ...
        if (ec)
            return "cannot create directory '" + display_path(parent) + "': " + ec.message();
    }
    ...
    std::string write_err;
    {
        ...
        if (!out) {
            ...
            out.flush();
            if (!out)
                write_err = "short write to '" + display_path(file) + "': " + errno_text();
        }
    }
    if (!write_err.empty()) {
...
std::string load_preset(const std::filesystem::path &file, PresetData &out) {
    if (const std::string derr = directory_target_error(file, "reading"); !derr.empty())
        return derr;
    std::string bytes;
    if (const std::string rerr = read_file_bytes(file, bytes); !rerr.empty())
        return rerr;

    QJsonParseError perr{};
    const QJsonDocument doc = QJsonDocument::fromJson(
        QByteArray(bytes.data(), static_cast<qsizetype>(bytes.size())), &perr);
...
list_presets(const std::filesystem::path &dir) {
    ...
    for (; it != end; it.increment(ec)) {
        ...
        if (name.empty() || name[0] == '.')
            continue; // QDir::Hidden 未开 → 不列隐藏文件
        if (!has_json_suffix(name))
            continue; // name filter "*.json"（大小写不敏感）
        std::error_code fec;
