...
- **T30 终轮 CI（tag 后的文档轮；产物非发布附件）**：run **`35729929908`**（head **`6f61abd8ead0e24228a7640b9be08ad601e7ff94`**，`push` 事件，2026-09-22T12:53:21Z→12:55:34Z）**四 job 全绿** —— `linux` **1m48s** / `ui-smoke` **1m47s** / `appimage` **1m57s** / `cache-gc` **8s**（job id 依次 `106752681805` / `106752681194` / `106752681641` / `106753366132`）。取证原文：`ui-smoke` job 冻结行 **`UI-SMOKE OK shots=8 pages=3`**（12:55:07Z，ctest `100% tests passed, 0 tests failed out of 1`）+ `linux` job `100% tests passed, 0 tests failed out of 23` 与 `SMOKE total=16 pass=16 fail=0`；`appimage` job 产物 **50,940,408 B** / sha256 `d8611955bef193c1ac80aceb885c7debf24ed3a220c7fbb10241fa9cb078dfb4` / 46 条 soname（体积与发行件相同但**指纹不同**，再次印证 §8-10 非字节可复现）。**本轮 head `6f61abd` = 发行提交 `4fb64c3` + T29/T30 文档提交，故该轮 artifact 仅验证用、非发布附件**；出厂口径仍是 tag `v0.1.0` → `4fb64c3` 的 Release 附件（见上一条）。
- **推送与覆盖边界（T27 闭合；计数锚点见 §1/§9）**：T22 之后的全部提交（T24/T25/T26/T27）**已推送**，T27 轮 `origin/main` = `d1b5fb7`；上表 CI 结论**即该轮实况**（run `35521800777`，head = 推送后的树），取代此前"撰写时未推送、结论对应 `a63acff`"的过渡说明。**发行提交 `4fb64c3` 与后续文档提交（T29 `672f695`、T30）均已推送**：`git push` 输出 `4fb64c3..6f61abd  main -> main`，**最终 `origin/main` = 本地 HEAD = `6f61abd`**（工作树 clean）。同一轮之前 `run 35521513855` 曾红在 `appimage` job（产物缺陷，非环境）→ 修复见 §6.5。
