...
QString state_text(pp::FileState s) {
    switch (s) {
    case pp::FileState::Queued:     return PageRun::tr("排队");
    case pp::FileState::Probing:    return PageRun::tr("探测");
    case pp::FileState::Decoding:   return PageRun::tr("解码");
    case pp::FileState::Orienting:  return PageRun::tr("旋转");
    case pp::FileState::Coloring:   return PageRun::tr("色彩");
    case pp::FileState::Flattening: return PageRun::tr("合成");
    case pp::FileState::Encoding:   return PageRun::tr("编码");
    case pp::FileState::Writing:    return PageRun::tr("写入");
    case pp::FileState::Done:       return PageRun::tr("完成");
    case pp::FileState::Skipped:    return PageRun::tr("跳过");
    case pp::FileState::Failed:     return PageRun::tr("失败");
    case pp::FileState::Cancelled:  return PageRun::tr("取消");
    }
    return QString();
}
...
QColor state_color(pp::FileState s) {
    switch (s) {
    case pp::FileState::Queued:     return QColor(0x80, 0x80, 0x80);   // #808080
    ...
    case pp::FileState::Writing:    return QColor(0x00, 0x88, 0xcc);   // #0088cc
    case pp::FileState::Done:       return QColor(0x22, 0xaa, 0x22);   // #22aa22
    case pp::FileState::Skipped:    return QColor(0x99, 0x99, 0x99);   // #999999
    case pp::FileState::Failed:     return QColor(0xdd, 0x33, 0x33);   // #dd3333
    case pp::FileState::Cancelled:  return QColor(0x99, 0x99, 0x99);   // #999999
    }
    return QColor(0x80, 0x80, 0x80);
}

bool is_terminal(pp::FileState s) {
    return s == pp::FileState::Done || s == pp::FileState::Skipped ||
           s == pp::FileState::Failed || s == pp::FileState::Cancelled;
}
...
QString format_duration(double ms) {
    if (ms >= 1000.0) return PageRun::tr("%1 s").arg(ms / 1000.0, 0, 'f', 1);
...
public:
    ...
    QVariant data(const QModelIndex& index, int role) const override {
        ...
        case Qt::ForegroundRole: return QBrush(r.color);
        case Qt::ToolTipRole:    return r.error.isEmpty() ? QVariant() : QVariant(r.error);
    ...
    QString error_at(int row) const {
        if (row < 0 || row >= rows_.size()) return QString();
...
void build_ui(PageRun* page, RunState* s) {
    ...
    QObject::connect(s->open_out, &QPushButton::clicked, page, [page, s]() {
        emit page->open_output_requested(s->out_root);
    });
    QObject::connect(s->open_logs, &QPushButton::clicked, page, [page]() {
        emit page->open_logs_requested();
    });
...
void PageRun::on_event(const pp::FileEvent& ev) {
    ...
    if (!is_terminal(ev.state)) return;

    if (ev.result) s->out_bytes += ev.result->out_bytes;   // 终态计入吞吐累计（不保存指针）
    switch (ev.state) {
    case pp::FileState::Done:      ++s->ok; break;
    case pp::FileState::Skipped:   ++s->skipped; break;
    case pp::FileState::Failed:    ++s->failed; break;
    case pp::FileState::Cancelled: ++s->cancelled; break;
    default: break;
    }
    ++s->terminal;
...
void PageRun::end_run(const pp::RunSummary& sum, const QString& out_root) {
    ...
    s->out_root = out_root;
    if (sum.cancelled > 0) s->cancel_requested = true;
