...
- **T30 终轮 CI（tag 后的文档轮；产物非发布附件）**：run **`35729929908`**（head **`6f61abd8ead0e24228a7640b9be08ad601e7ff94`**，`push` 事件，2026-09-22T12:53:21Z→12:55:34Z）**四 job 全绿** —— `linux` **1m48s** / `ui-smoke` **1m47s** / `appimage` **1m57s** / `cache-gc` **8s**（job id 依次 `106752681805` / `106752681194` / `106752681641` / `106753366132`）。取证原文：`ui-smoke` job 冻结行 **`UI-SMOKE OK shots=8 pages=3`**（12:55:07Z，ctest `100% tests passed, 0 tests failed out of 1`）+ `linux` job `100% tests passed, 0 tests failed out of 23` 与 `SMOKE total=16 pass=16 fail=0`；`appimage` job 产物 **50,940,408 B** / sha256 `d8611955bef193c1ac80aceb885c7debf24ed3a220c7fbb10241fa9cb078dfb4` / 46 条 soname（体积与发行件相同但**指纹不同**，再次印证 §8-10 非字节可复现）。**本轮 head `6f61abd` = 发行提交 `4fb64c3` + T29/T30 文档提交，故该轮 artifact 仅验证用、非发布附件**；出厂口径仍是 tag `v0.1.0` → `4fb64c3` 的 Release 附件（见上一条）。**末轮（T30 回填提交后）文档轮 run `35730278620`**（head **`87acab08d0ab7f682d11cbcabe33e492747c03ea`**，`push` 事件，2026-09-22T12:56:42Z→12:59:04Z）**conclusion=success，四 job 全绿** —— `linux` **2m05s** / `ui-smoke` **1m30s** / `appimage` **1m59s** / `cache-gc` **9s**（job id 依次 `106753849456` / `106753849480` / `106753849250` / `106754597569`），冻结行再次取得 `UI-SMOKE OK shots=8 pages=3`（12:58:11Z，ctest `… out of 1`）；**同属 tag 后的文档轮，产物非发布附件**。
- **推送与覆盖边界（T27 闭合；计数锚点见 §1/§9）**：T22 之后的全部提交（T24/T25/T26/T27）**已推送**，T27 轮 `origin/main` = `d1b5fb7`；上表 CI 结论**即该轮实况**（run `35521800777`，head = 推送后的树），取代此前"撰写时未推送、结论对应 `a63acff`"的过渡说明。**发行提交 `4fb64c3` 与后续文档提交均已推送**（两次推送：`4fb64c3..6f61abd`、`6f61abd..87acab0`，均 `main -> main`）；**tag 后文档修订链 = `672f695`（T29）→ `6f61abd` → `87acab0`（T30；各自 CI run 见本节上条）**，其后新增文档提交不再逐一登记。**本报告不再声明"最终提交 hash"**（任何此类声明在写出后必然过期）；**仓库当前状态一律以 `git log origin/main` 为准**。同一轮之前 `run 35521513855` 曾红在 `appimage` job（产物缺陷，非环境）→ 修复见 §6.5。
...
| 批次 | 范围 | 提交数 | 关键产出 | 收口提交 |
|---|---|---|---|---|
| M0 | 骨架/冻结接口/语料/构建引导 | 16 | 设计文档、任务书、vcpkg 清单 + jpegli overlay、27 fixture 语料 | `d59fe46`（M0 收口）等（首提交 `75fca98`） |
| M1a | 引擎（core/decode/codecs/pipeline/scheduler/dev harness） | 44 | 8 编码器、色彩管理、Exif 无损重写、`--dev`、金样 9 对、15 冻结头 | `e4fe158`（M1a 收口） |
| M1b | 界面（三页主窗口/参数表单/地图/编辑器/设置预设/UI 冒烟） | 40 | `--ui-smoke` 走查 8/8、16 冻结头、UI 自验 1000 断言 | `b97eff2`（M1b 收口） |
...
> **计数口径（T30 冻结；全表统一锚点重算）**：本表"提交数"一律按**同一发行锚点**实测，边界 commit 明确如下 —— **M0** = 首提交 `75fca98`..M0 收口 `d59fe46` = **16**；**M1a** = ..M1a 收口 `e4fe158` = **44**；**M1b** = ..M1b 收口 `b97eff2` = **40**；**M2** = `ab5a88a^..`（M2 任务书提交起）发行提交 `4fb64c3` = **55**；合计 **155** = `git rev-list --count 4fb64c3`（tag `v0.1.0`），等价于 `git rev-list --count ab5a88a^..4fb64c3` = 55。**早期 M0/M1a 报告采用的是不同的边界约定**（合并/边界归入法不同），故本节按上述锚点统一重算；如需追溯旧值见 `docs/m0-tasks.md` / `docs/m1-report.md` 对应段落。**不再按会随文档轮漂移的 HEAD 记数**：`4fb64c3` 之后仅 T29/T30 文档提交，不影响发行产物与 tag。
