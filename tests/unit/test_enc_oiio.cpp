...
bool read_spec(const fs::path& p, OIIO::ImageSpec& spec, std::string& err) {
    ...
    if (!in) { err = "ImageInput::open failed: " + OIIO::geterror(); return false; }
    spec = in->spec();
    in->close();
    return true;
}
...
bool read_datetime_original(const fs::path& p, std::string& value, std::string& err) {
    try {
        ...
        if (it == ex.end()) { err = "Exif.Photo.DateTimeOriginal missing"; return false; }
        value = it->toString();
        return true;
...
void test_registry() {
    ...
    for (const auto& c : combos) {
        ...
        if (enc) check(enc->format().id == c[0], "registry/format-id",
...
void test_png_roundtrip() {
    ...
    {   // E3: 12-bit is not a PNG capability
        ...
        check(!r.error.empty() && r.bytes == 0, "png/bitdepth12/error", "expected error, got " + r.error);
    }
}
...
void test_tiff_roundtrip_and_compression() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (std::string(names[i]) == "zip") params["deflate_level"] = int64_t(9);
        OIIO::ImageBuf buf = make_buf(64, 64, 3, 0.0f);
        ...
        info(std::string("tiff ") + names[i] + ": " + num(static_cast<int64_t>(r.bytes)) + " bytes");
    }
...
void test_encoder_shared_across_threads() {   // E1
    ...
    for (int t = 0; t < kThreads; t++) {
        pool.emplace_back([&, t]() {
            OIIO::ImageBuf buf = make_buf(64, 64, 3, 0.0f);
            const pp::EncodeResult r = run_encode(*enc, buf, 8, params, meta,
                                                  dir / ("conc_" + num(t) + ".png"));
            errors[t] = r.error;
            bytes[t] = r.bytes;
        });
...
void test_introspection() {
    ...
    for (const auto& b : heif) {
        ...
        check(!b.techs.empty() && !b.techs.front().params.empty(),
              "introspect/heif/params>0", b.id + " has no params");
        const auto& params = b.techs.front().params;
        const bool has_quality =
            std::any_of(params.begin(), params.end(),
                        [](const pp::ParamDef& p) { return p.key == "quality"; });
    ...
    for (const auto& b : avif) {
        check(!b.techs.empty() && !b.techs.front().params.empty(),
              "introspect/avif/params>0", b.id + " has no params");
        const auto& params = b.techs.front().params;
        const bool has_quality =
            std::any_of(params.begin(), params.end(),
                        [](const pp::ParamDef& p) { return p.key == "quality"; });
...
void test_bitdepth_probe() {
    ...
    check(heif_x265 == "8" || heif_x265 == "8,10" || heif_x265 == "8,10,12",
          "probe/heif-format", "'" + heif_x265 + "'");
...
void test_heif_and_avif_metadata() {
    ...
    meta.xmp_rdf =
        ...
        "<dc:title><rdf:Alt><rdf:li xml:lang=\"x-default\">" + xmp_marker + "</rdf:li></rdf:Alt>"
    ...
    std::vector<Case> cases = {{"heif", "x265", "heic", 8}, {"avif", "svt-av1", "avif", 8},
                               {"avif", "libaom", "avif", 8}};
    if (heif_depths.find("10") != std::string::npos) cases.push_back({"heif", "x265", "heic", 10});
    if (svt_depths.find("10") != std::string::npos) cases.push_back({"avif", "svt-av1", "avif", 10});
    cases.push_back({"avif", "libaom", "avif", 10});   // measured: libaom handles 10-bit

    for (const Case& c : cases) {
        const std::string tag = std::string(c.format) + "/" + c.backend + "/" +
                                num(c.bitdepth) + "bit";
        ...
        const fs::path out = dir / (std::string(c.format) + "_" + c.backend + "_" +
                                    num(c.bitdepth) + "." + c.ext);
        ...
        info(tag + ": " + num(static_cast<int64_t>(r.bytes)) + " bytes, DateTimeOriginal=" +
             value);
    }
    ...
    {   // measured limitation: svt-av1 10-bit + alpha -> error naming libaom
        ...
        if (svt10.find("10") != std::string::npos) {
            ...
            check(r.error.find("libaom") != std::string::npos,
                  "avif/svt-10bit-alpha/names-libaom", "error was '" + r.error + "'");
