...
public:
    FitOnShow(QObject *parent, std::function<void()> fit) : QObject(parent), fit_(std::move(fit)) {}
...
SettingsDialog::SettingsDialog(const pp::AppSettings &current, QWidget *parent) : QDialog(parent) {
    ...
    connect(flatten, &QSlider::valueChanged, swatch, [swatch](int v) { apply_swatch(swatch, v); });
    ...
    auto *version =
        new QLabel(tr("PhotoPipeline %1").arg(QLatin1String(PP_VERSION_STRING)), page_about);
