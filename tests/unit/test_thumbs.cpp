...
std::string dims(const pp::ThumbImage& t) {
    return std::to_string(t.width) + "x" + std::to_string(t.height) + " rgba=" +
           std::to_string(t.rgba.size());
}
...
bool gray_is_neutral(const pp::ThumbImage& t) {
    for (std::size_t i = 0; i + 3 < t.rgba.size(); i += 4) {
        if (t.rgba[i] != t.rgba[i + 1] || t.rgba[i] != t.rgba[i + 2]) return false;
    }
    return true;
}
...
fs::path build_preview_fixture(const fs::path& corpus, const fs::path& dir) {
    ...
    fs::copy_file(corpus / "meta" / "exif_full.jpg", host, fs::copy_options::overwrite_existing, ec);
    if (ec) return {};
    try {
...
int main() {
    ...
    // ---- base/rgb8.png @96: probe ok, no upscale (64x64 source), RGBA size, no preview ----
    {
        ...
        check(o.info.width == 64 && o.info.height == 64 && o.info.channels == 3 &&
                  o.info.format == "png",
              "rgb8/probe-info", "w=" + std::to_string(o.info.width) + " h=" +
                                     std::to_string(o.info.height) + " ch=" +
                                     std::to_string(o.info.channels) + " fmt=" + o.info.format);
    }

    // ---- base/rgb8.png @32: downscale honours the target ----
    {
        const pp::ThumbOutcome o = pp::make_thumbnail(corpus / "base/rgb8.png", 32);
        check(o.probe_ok && o.thumb.width == 32 && o.thumb.height == 32 && rgba_size_matches(o.thumb),
    ...
    // ---- edge/ corrupt negatives: probe fails, no thumbnail, error reported ----
    {
        ...
        for (const char* rel : bad) {
            ...
            check(o.thumb.rgba.empty(), std::string("corrupt/") + rel + "/no-thumb",
                  dims(o.thumb));
        }
    }
    ...
    // ---- meta/exif_full.jpg: probe ok; thumbnail required, from_embedded not asserted ----
    {
        ...
        check(rgba_size_matches(o.thumb) && o.thumb.width <= 96 && o.thumb.height <= 96,
              "exif-jpg/thumb-shape", dims(o.thumb) + " from_embedded=" +
                                          (o.thumb.from_embedded ? "true" : "false"));
    }
    ...
    // ---- embedded preview path (synthetic Exif IFD1 thumbnail) ----
    {
        ...
        if (!host.empty()) {
            ...
            check(small.thumb.from_embedded && small.thumb.width == 24 &&
                      small.thumb.height == 24,
